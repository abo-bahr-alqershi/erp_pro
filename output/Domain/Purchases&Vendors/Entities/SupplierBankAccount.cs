public class SupplierBankAccount
{
    /// <summary> كود المورد </summary>
    public string? VendorCode { get; private set; }
    public long? BankNo { get; private set; }
    /// <summary> رقم الحساب البنكي </summary>
    public string? BankAccount { get; private set; }
    /// <summary> اسم البنك </summary>
    public string? BankName { get; private set; }
    /// <summary> رمز السويفت </summary>
    public string? SwiftCode { get; private set; }
    public short? CountryNo { get; private set; }
    public long? CityNo { get; private set; }
    /// <summary> اسم المستفيد </summary>
    public string? BeneficiaryName { get; private set; }
    /// <summary> مفتاح البنك </summary>
    public string? BankKey { get; private set; }
    /// <summary> رقم الآيبان </summary>
    public string? IBANNo { get; private set; }
    /// <summary> رمز العملة </summary>
    public string? CurrencyCode { get; private set; }
    public int? AddedUserId { get; private set; }
    /// <summary> تاريخ الإضافة </summary>
    public DateTime? AddedDate { get; private set; }
    public int? UpdatedUserId { get; private set; }
    /// <summary> تاريخ آخر تعديل </summary>
    public DateTime? UpdatedDate { get; private set; }
    public long? UpdateCount { get; private set; }
    public string VDegreeAName { get; private set; }
    public string? VDegreeEName { get; private set; }
    public int? AddedUserId { get; private set; }
    /// <summary> تاريخ الإضافة </summary>
    public DateTime? AddedDate { get; private set; }
    public int? UpdatedUserId { get; private set; }
    /// <summary> تاريخ آخر تعديل </summary>
    public DateTime? UpdatedDate { get; private set; }
    public long? UpdateCount { get; private set; }
    public long? PrRep { get; private set; }
    public string? AdTrmnlNm { get; private set; }
    public string? UpTrmnlNm { get; private set; }
    /// <summary> كود المورد </summary>
    public string? VendorCode { get; private set; }
    public string? ACode { get; private set; }
    public bool? Inactive { get; private set; }
    public DateTime? InactiveDate { get; private set; }
    public string? InactiveRes { get; private set; }
    public int? InactiveUId { get; private set; }
    /// <summary> تاريخ الإضافة </summary>
    public DateTime? AddedDate { get; private set; }
    public int? AddedUserId { get; private set; }
    public int? UpdatedUserId { get; private set; }
    /// <summary> تاريخ آخر تعديل </summary>
    public DateTime? UpdatedDate { get; private set; }
    public long? UpdateCount { get; private set; }
    public int? PrRep { get; private set; }
    public string? AdTrmnlNm { get; private set; }
    public string? UpTrmnlNm { get; private set; }
}